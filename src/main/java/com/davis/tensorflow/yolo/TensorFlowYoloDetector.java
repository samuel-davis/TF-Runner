...
//  // from files.
//  private static final double[] YOLO_VOC_ANCHORS = {
//    1.08, 1.19,
//    3.42, 4.41,
//    6.63, 11.38,
//    9.42, 5.11,
...
//    Image
//    bitmap.getPixelsXy(intValues, 0, bitmap.getWidth(), 0, 0, bitmap.getWidth(), bitmap.getHeight());
//
...
//
//          final float w = (float) (Math.exp(output[offset + 2]) * YOLO_VOC_ANCHORS[2 * b + 0]) * blockSize;
//          final float h = (float) (Math.exp(output[offset + 3]) * YOLO_VOC_ANCHORS[2 * b + 1]) * blockSize;
//
//          final RectF rect =
//              new RectF(
//                  Math.max(0, xPos - w / 2),
